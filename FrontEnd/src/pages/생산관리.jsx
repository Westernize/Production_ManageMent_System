  ...
  const renderCancelPanel = () => (
    <div className="form-section">
      ...
      <div className="form-row">
        ...
        <span>
          {selectedProd?.updDt
            ? new Date(selectedProd.updDt).toLocaleString(undefined, { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })
            : '-'}
        </span>
      </div>
    ...
    <div className="order-management-container">
      <div className="customer-header">
        ...
        <div className="header-buttons">
          <button className="excel-btn prod-create-btn" onClick={handleCreateClick}>새 생산계획</button>
        </div>
      </div>
      ...
      <div className="order-content-layout">
              ...
              <div className="advanced-filter-panel">
                <div className="filter-row">
                  ...
                  <div className="filter-field filter-field-keyword">
                    ...
                    <div className="keyword-row" style={{display:'flex', alignItems:'center', gap:12}}>
                      <SearchBar
                        searchOptions={[
                  ...
                  <div className="filter-field filter-field-cancel">
                  ...
                  <div className="filter-actions">
                    <button className="filter-search-btn" onClick={() => { setAppliedSearchTerm(searchTerm); setIsFilterOpen(false); }}>검색</button>
                    <button className="filter-reset-btn" onClick={() => {
                        setStartDate(''); setEndDate(''); setSearchTerm(''); setAppliedSearchTerm(''); setStatusFilter('ALL'); setIncludeCanceled(true);
                      }}>초기화</button>
                  </div>
                </div>
              </div>
        ...
        <div className="order-detail-wrapper">
          <div className="production-detail-panel">
            {selectedProd ? (
              <>
                ...
                <div className="detail-meta-bar">
                  ...
                  <div className="meta-section">
                      ...
                      {(selectedProd.status === '02' || selectedProd.status === '03') && (
                          <button className="erp-button erp-button-default" onClick={handlePrev}>&lt; 이전단계</button>
                      )}
                      {selectedProd.status < '07' && selectedProd.status !== '09' && 
                          <button className="erp-button erp-button-primary" onClick={handleNext}>다음단계 &gt;</button>
                      }
                  </div> 
                </div>
          ...
          {selectedProd && (
            ['08','09'].includes(selectedProd.status) ? (
              <div className="production-detail-footer cancelled">
                <span className="cancel-text">계획이 취소되었습니다.</span>
              </div>
            ) : (
              <div className="production-detail-footer">
                {!['07', '08', '09'].includes(selectedProd.status) && (
                  <button className="erp-button erp-button-primary" onClick={() => saveProdToDb()} style={{ backgroundColor: '#16a34a', borderColor: '#16a34a' }}>계획 저장</button>
                )}
